...
object FunctionalProgramming extends App {
  ...
  bob.apply(43)
  bob(43) // INVOKING bob as function === bob.apply
  /*
  ...
  val aFilteredList = List(1,2,3,4,5).filter(_ <= 3) // equivalent to x =< 3
  ...
  val anAddedSed = aSet + 5 // Set(1,2,3,4,5)
  val aRemovedSet = aSet - 3 // Set(1,2,4)

  // ranges
  val aRange = 1 to 1000

  val twoByTwo = aRange.map(x=> 2 * x).toList // Lsit(2,4,6,8,....,200)

  // tuples = groups of values under the same value
  val aTuple = ("Bon Javi","Rock",1982)
